...
class TestEmbeddingDistributionManager:
    ...
    def test_distribute_embeddings_valid(self, mock_to, distribution_manager, mock_gpu_manager):
        """Test successful embedding distribution"""
        embeddings = torch.randn(100, 128)
        
        # Mock tensor.to() to return the tensor itself (simulating successful GPU transfer)
        mock_to.side_effect = lambda device: embeddings[:50] if '0' in device else embeddings[50:]
    ...
    def test_validate_distribution_invalid_gpu(self, distribution_manager, mock_gpu_manager):
        ...
        tensor = Mock()
        tensor.device = Mock()
        tensor.device.__str__ = Mock(return_value='cuda:0')
        tensor.shape = (100, 128)
        tensor.dtype = torch.float32
        
        part = EmbeddingPart(0, tensor, 0, 100)
        dist_emb = DistributedEmbeddings([part], 100, 128)
        
        # Mock GPU validation to return empty list (no available GPUs)
        mock_gpu_manager.get_available_gpu_ids.return_value = []
    ...
    def test_validate_distribution_wrong_device(self, distribution_manager, mock_gpu_manager):
        ...
        tensor = Mock()
        tensor.device = Mock()
        tensor.device.__str__ = Mock(return_value='cpu')  # Wrong device type
        tensor.shape = (100, 128)
        tensor.dtype = torch.float32
        ...
        mock_gpu_manager.get_available_gpu_ids.return_value = [0]
    ...
    def test_cleanup_distribution(self, distribution_manager, mock_gpu_manager):
        ...
        # Verify current distribution was cleared (since we passed the same distribution)
